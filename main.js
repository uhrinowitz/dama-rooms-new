...
socket.on('start-session', (data) => {
	...
	console.log(data);
		...
		var session_id = uuid(); //generating the sessions_id and then binding that socket to that sessions 
		socket.room = session_id;
		socket.join(socket.room, (res) =>{
...
socket.on('newRoomCreated', ()=>{
	roomsCreated++;
	socket.to('lobby').emit('newRoomCreated', roomsCreated);
	console.log('telling client about rooms: ' + roomsCreated)
});
